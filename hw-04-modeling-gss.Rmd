...
gss16 %>%
fct_collapse(advfront, agree = c("Strongly agree", "Agree"), Notagree = c("Disagree", "Strongly disagree", "Dont know")) 
  
advfront <- factor(advfront, levels = c("Agree", "Notagree"))
summary(advfront)
...
```{r, eval = TRUE}
  Liberal <- gss16 %>%
  filter(str_detect(polviews, "[Ll]iberal")) %>%
  count(polviews) 

  Conservative <- gss16 %>%
  filter(str_detect(polviews, "[Cc]onservative")) %>%
  count(polviews)
  
  Moderate <- gss16 %>%
  filter(str_detect(polviews, "[Mm]oderate")) %>%
  count(polviews)
```
***There are 928 conservatives, 796 liberals, and 1032 moderates.***
...
```{r eval=TRUE}
gss16_spec2 <- logistic_reg() %>%
  set_engine("glm")
```

```{r eval=TRUE}
gss16_wflow_2 <- workflow() %>%
  add_model(gss16_spec2) %>%
...
***I'm not sure because neither of my models are working.***
...
harassbi <- gss16 %>%
  select(harass5) %>%
  drop_na() #$>$
#harassbi$harass5 <-gsub("Does not apply (i do not have a job/superior/co-worker)","",as.character(harassbi$harass5))

count(harassbi)
```

***By simple math I can see that there are 1,373 yes and no answers excluding the "Does not apply" answers. There are 1,135 "no" responses and 334 "yes" responses.***
...
***We could use bootstrapping to estimate this by re-sampling the Harass5 variable with the same sample size as the original while also replacing it with the original sample. Then we would use that data to calculate a bootstrap statistic, in this case the proportion of "yes" to "no" in regards to harassment at work. We would then repeat these steps thousands of times to create a bootstrap distribution.***
...
***I would expect a 90% confidence interval to be narrower (more precise), because its estimate has less chance of being correct while also being a more precise estimate. For example, it's like saying there's a 90% chance that between 35-40% of Americans have been harassed at work, instead of there being a 95% chance that 35-45% of Americans have been harassed at work.***
